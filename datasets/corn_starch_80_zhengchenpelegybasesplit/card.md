# Datasheet — CORN — Corn_Starch_80_ZhengChenPelegYbaseSplit

_Generated from the dataset descriptor and identity card (Datasheets for Datasets, Gebru et al. 2021)._

## Motivation

- **Domain / purpose:** corn
- **Description:** CORN dataset (regression). Sample: Corn kernel. Trait: Starch content. Split: Y sorted. Auto-generated descriptor (verify before publication).
- **Contributor:** https://eigenvector.com/resources/data-sets/

## Composition

- **Instances:** 80 spectra × 700 wavelengths, 1 source(s).
- **Modality / instrument:** NIR — *Not specified.* *Not specified.* (firmware *Not specified.*).
- **Spectral axis:** [1100.0, 2498.0] nm; signal type unknown.
- **Targets:** Starch content (regression)
- **Contains missing values:** False

## Statistics

- **PCA / dimensionality:** effective rank 1.02; 1 PC(s) → 95% variance, 1 → 99%.
- **Train↔test target shift:** std. mean diff -0.0206, KS 0.0312 (p 1), Wasserstein 0.0732

## Collection process

- **Collection date:** *Not specified.*
- **Reference method:** Starch content
- **Lab protocol:** *Not specified.*
- **Consent / ethics status:** Not assessed (auto-generated; verify before release).
- **Anonymization status:** Not assessed (auto-generated; verify before release).

## Preprocessing / cleaning / labeling

- **Conversion:** tabular (status *Not specified.*).
- **Known exclusions:** *Not specified.*

## Uses

- **Permitted use:** Research and benchmarking.
- **Citation:** http://dx.doi.org/10.1016/j.cclet.2014.10.023

## Distribution

- **License:** LicenseRef-unknown
- **Visibility:** restricted
- **Confidentiality class:** internal
- **DOI:** *Not specified.*
- **Redistribution rights:** Original source licence applies; verify before redistribution.

## Maintenance

- **Owner / steward:** https://eigenvector.com/resources/data-sets/
- **Version:** 0.1.0
- **Access policy:** Local use; not published to Dataverse.
