...
def _dataverse_client(instance: str | None):
...
def publish(
    ...
    Restricted/embargo datasets upload with access-restricted files. On success the minted DOI +
    version are written back into the descriptor, the manifest records the Dataverse file ids +
    native checksums (so private access by token works), and the card/Croissant + catalog are refreshed.
    """
    from nirs4all_datasets.catalog import build_catalog
    ...
    build_card(dataset_dir, _descriptor(root, dataset_id))  # reload with the DOI so card/Croissant carry it
    build_catalog(root)
