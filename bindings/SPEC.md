...
SHA-256-verifies against the index; and caches it pooch-style. The scientific
...
The v0 surface that crosses the C ABI is **strings (JSON) + filesystem paths**:
`resolve`, `fetch`, `verify_cached`. **No materialized arrays and no opaque
...
(`pyarrow` / R `arrow` / MATLAB `parquetread` / `parquet-wasm`). The returned
...
(`kind`/`mode`/`locator`/`access`); and the sanitized `descriptor`.
...
/ `n4ds_fetch` / `n4ds_verify_cached`) and the opaque context. Each such string
...
- `n4ds_abi_version()` returns the ABI version string (owned; free with
  ...
  currently `0.1.0`) is **independent of the crate semver** (`0.2.0-alpha.1`) —
...
- [`crates/nirs4all-datasets-capi/abi/expected_symbols_{linux,macos,windows}.txt`](../crates/nirs4all-datasets-capi/abi)
  snapshot the exported symbol set (the nine `n4ds_*` symbols); the ABI-check CI
  ...
  node `N4DS_1`, wired in `build.rs`) that makes everything else `local`, on
...
The canonical v0 surface is exactly these nine symbols:
...
- **Python = pyo3-native** and **embedded** (formats/io model). The pyo3 extension
  ...
  `abi_version` / `resolve` / `fetch` / `verify_cached`; `fetch` releases the GIL
  for the blocking network duration. Its idiomatic layer
...
- **R = C-ABI-first** (`extendr`/C shim over the C ABI; CRAN target — see
  ...
  (`resolve`/`fetch`/`verify_cached`); rated *medium* (native-lib packaging,
